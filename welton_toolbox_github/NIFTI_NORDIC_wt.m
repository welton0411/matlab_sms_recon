function  NIFTI_NORDIC_wt(fn_magn_in,fn_phase_in,fn_out,ARG)
% fMRI
...
if ARG.magnitude_only~=1
    ...
    if ARG.use_generic_NII_read~=1
        tmpvol=load_nii(fn_magn_in);
        %I_M=abs(single(niftiread(fn_magn_in)));
        I_M=abs(single(tmpvol.img));
        tmpvol=load_nii(fn_phase_in);
        %I_P=single(niftiread(fn_phase_in));
        I_P=single(tmpvol.img*pi/180);
    else
        try
...
if ( ARG.save_gfactor_map==2 )  | ( ARG.save_gfactor_map==1 )
    ...
    nii=make_nii(g_IMG);
    save_nii(nii,[ARG.DIROUT 'gfactor_' fn_out(1:end) '.nii']);
...
if isfield(ARG,'make_complex_nii')
    ...
    nii=make_nii(IMG2_tmp);
    save_nii(nii,[ARG.DIROUT fn_out 'magn.nii']);
    ...
    nii=make_nii(IMG2_tmp);
    save_nii(nii,[ARG.DIROUT fn_out 'phase.nii']);
else
