function twix_obj = mapVBVD_wt(filename,varargin)
...
while k <= numel(varargin)
    ...
    switch lower(varargin{k})
        ...
        case {'rtfeedback','dortfeedback'} %% added by WeiTang on July 12,2021
            if numel(varargin) > k && ~ischar(varargin{k+1})
                arg.bReadRTfeedback = logical(varargin{k+1});
                k = k+2;
            else
                arg.bReadRTfeedback = true;
                k = k+1;
            end
        case {'rolineinfo'} %% added by WeiTang on Nov 9,2025
            if numel(varargin) > k && ~ischar(varargin{k+1})
                ROLineInfo = varargin{k+1};
                k = k+2;
            else
                %ROLineInfo = [];
                %k = k+1;
                error('Error! structure of RO line information is missing.\n');
            end
        otherwise
...
for s=1:NScans
    ...
    rstraj = [];
    if arg.bReadHeader
      if(~isempty(ROLineInfo))
        [twix_obj{s}.hdr, rstraj] = read_twix_hdr_wt(fid, ROLineInfo);
      else
        [twix_obj{s}.hdr, rstraj] = read_twix_hdr_wt(fid);
      end
    end
