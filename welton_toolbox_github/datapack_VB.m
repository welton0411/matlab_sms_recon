function [noise_data refscan_data image_data]=datapack_VB(twix_obj,varargin)
...
ice_obj.flag_train_scan=flag_train_scan;
...
noise_rawdata=twix_obj.noise.unsorted();
noise_data=reshape(permute(noise_rawdata,[1 3 2]),[],twix_obj.noise.NCha);

PEshift=  round(ice_obj.Npe/twix_obj.hdr.Config.PhasePartialFourierFactor);
PartialFourtierRatio=1-1/double(twix_obj.hdr.Config.PhasePartialFourierFactor);

for f = fieldnames( twix_obj ).'
   if(~(strcmp(f,'hdr') || strcmp(f,'vop')))
      fprintf('Reading %s ... ',f{1});
      str=sprintf('%s_rawdata = twix_obj.(f{1}).unsorted();',f{1});
      eval(str);
      fprintf('done!\n');
   end
end
...
if(flag_phase_cor)
   ...
   %keyboard;
   phasecor_rawdata=fftshift(fft(ifftshift(phasecor_rawdata,1),[],1),1);
end
image_rawdata=fftshift(fft(ifftshift(image_rawdata,1),[],1),1);

n_acqLperKplane=twix_obj.hdr.Config.RawLin;
nav_echo=[];
tmpnum_kplane=numel(find(twix_obj.image.Rep==1))/twix_obj.hdr.Config.RawLin;
num_ref_kplane=tmpnum_kplane-twix_obj.hdr.Config.RawSlc/ice_obj.sms_factor;
%keyboard;
%%%% Load reference scan %%%%
if(ice_obj.flag_train_scan==1)
   refscan_data=zeros(ice_obj.Npe, twix_obj.hdr.Config.BaseResolution, twix_obj.hdr.Config.NSlc, twix_obj.image.NCha, num_slc_pae, 'single');
   ice_obj.Nfe=twix_obj.hdr.Config.BaseResolution;
   %ice_obj.Npe=twix_obj.hdr.Config.PhaseEncodingLines;
   ice_obj.Nch=twix_obj.image.NCha;
   ice_obj.NSlc=twix_obj.hdr.Config.NSlc;
   num_refAcqSlc=twix_obj.hdr.Config.RawSlc;
         
   for kidx=1:num_ref_kplane,
      pae_idx=ceil(kidx/(num_refAcqSlc*ice_obj.pat));
      kL_offset=(kidx-1)*n_acqLperKplane;
      ref_slcidx=unique(twix_obj.image.Sli(kL_offset+1:kL_offset+n_acqLperKplane));
      ...
      if(flag_phase_cor)
         phcor_slcidx=unique(twix_obj.phasecor.Sli((kidx-1)*3+1:kidx*3));
         ...
         for nvidx=1:3,
            nav_echo{phcor_slcidx, twix_obj.phasecor.Seg((kidx-1)*3+nvidx)}=phasecor_rawdata(:,:,(kidx-1)*3+nvidx);
         end
         
         calc_epi_phase_correction(nav_echo, phcor_slcidx);
         
         oddline_idx=find(twix_obj.image.Seg(kL_offset+1:kL_offset+n_acqLperKplane)==2);
         ...
         %fprintf('kidx=%d',kidx);
         %keyboard;
         refscan_data(PEshift+twix_obj.image.Lin(kL_offset+1:kL_offset+n_acqLperKplane),:,ref_slcidx,:,pae_idx)=reshape(permute(tmp_kplane,[3 1 2]),[n_acqLperKplane, twix_obj.hdr.Config.BaseResolution, 1, twix_obj.image.NCha, 1]);
      else
         %keyboard;
         refscan_data(PEshift+twix_obj.image.Lin(kL_offset+1:kL_offset+n_acqLperKplane),:,ref_slcidx,:,pae_idx)=reshape(permute(image_rawdata(:,:,kL_offset+1:kL_offset+n_acqLperKplane),[3 1 2]),[n_acqLperKplane, twix_obj.hdr.Config.BaseResolution, 1, twix_obj.image.NCha, 1]);
      end
   end %end of for kidx=1:num_ref_kplane,
   
   refscan_data=refscan_data(1:ice_obj.Npe,:,:,:,:);
   refscan_data=fftshift(fft(ifftshift(refscan_data,1),[],1),1);
end
%twix_obj.hdr.Dicom.lAccelFactPE

image_data=zeros(ice_obj.Npe, twix_obj.hdr.Config.BaseResolution, twix_obj.hdr.Config.NSlc, twix_obj.image.NCha, num_slc_pae, twix_obj.hdr.Config.RawRep/num_slc_pae,'single');
%n_acqLperKplane=twix_obj.hdr.Config.RawLin;
num_img_kplane=twix_obj.image.NAcq/n_acqLperKplane-num_ref_kplane;
num_imgAcqSlc=numel(unique(twix_obj.image.Sli(num_ref_kplane*n_acqLperKplane+1:end)));
for kidx=1:num_img_kplane,
   ...
   img_slcidx=unique(twix_obj.image.Sli(kL_offset+1:kL_offset+n_acqLperKplane));
   ...
   if(flag_phase_cor)
      phcor_slcidx=unique(twix_obj.phasecor.Sli((kidx+num_ref_kplane-1)*3+1:(kidx+num_ref_kplane)*3));
      if(numel(phcor_slcidx)>1)
         fprintf('ERROR! The 3 navigator echoes should belong to the same slice.\n');
         return;
      end
      ...
      for nvidx=1:3,
         nav_echo{phcor_slcidx, twix_obj.phasecor.Seg((kidx+num_ref_kplane-1)*3+nvidx)}=phasecor_rawdata(:,:,(kidx+num_ref_kplane-1)*3+nvidx);
      end
      ...
      image_data(PEshift+twix_obj.image.Lin(kL_offset+1:kL_offset+n_acqLperKplane),:,img_slcidx,:,pae_idx,rep_idx)=reshape(permute(tmp_kplane,[3 1 2]),[n_acqLperKplane, twix_obj.hdr.Config.BaseResolution, 1, twix_obj.image.NCha, 1, 1]);
   else
      image_data(PEshift+twix_obj.image.Lin(kL_offset+1:kL_offset+n_acqLperKplane),:,img_slcidx,:,pae_idx,rep_idx)=reshape(permute(image_rawdata(:,:,kL_offset+1:kL_offset+n_acqLperKplane),[3 1 2]),[n_acqLperKplane, twix_obj.hdr.Config.BaseResolution, 1, twix_obj.image.NCha, 1, 1]);
   end
end %end of for kidx=1:num_ref_kplane,

image_data=image_data(1:ice_obj.Npe,:,:,:,:,:);
image_data=fftshift(fft(ifftshift(image_data,1),[],1),1);
...
x=repmat(x,[1, sz_kplane(2), sz_kplane(3)]);
%keyboard;
offset=repmat(reshape(ice_obj.nav_phase_offset,[1 sz_kplane(2) 1]),[sz_kplane(1),1,sz_kplane(3)]);
