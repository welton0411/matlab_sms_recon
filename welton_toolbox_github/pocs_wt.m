function [im, kspFull] = pocs_wt( ksp, iter, watchProgress )
%Partial-Fourier Reconstruction with POCS
%
% [im, kspFull] = pocs_wt( kspIn, iter, watchProgr )
%
% === Input ===
%
...
% =========================================================================
    ...
    %fprintf( '%s\n%s\n%s   %s', b, b(1), b(1), progress_str ) %marked by WeiTang
    edging = sprintf( '\n%s\n%s', b(1), b );
    %fprintf( edging ) %marked by WeiTang
    % ( ===================================================================
    ...
    for ii = double(~watchProgress) : iter
        if ii > 0
            ...
            %fprintf([repmat('\b',1,prevLength) '%s' '%s'], progress_str, edging ); %marked by WeiTang
        end % if ii > 0
    ...
    if wasAddedCoilDim
      ...
      im = flipdim(im, 3);
      im = flipdim(im, 4);
      im = im*size(im,2)*size(im,3)*size(im,4);
      im = circshift(im,[0 0 1 1]);
    end
...
function [ centreLine, idxSym ] = findSymSampled( ksp, pfDim, Nsmp )
    ...
    if any(idxSym < 1)    ||   any(idxSym > sz(pfDim))
       ...
       fprintf('pocs:BadDataProperty, Symmetric part of k-space out of bounds.\nThe maximum k-space intensity is at index %g whereas it should be centred => near %g.\nThe way, zerofilling was done is probably wrong.\n', centreLine, round(sz(pfDim)/2) );
       centreLine = floor(sz(pfDim)/2)+1;
       startSym = centreLine - (Nsmp - centreLine);    % start of our symmetric sampling
       endSym   = centreLine + (Nsmp - centreLine);    % end of symmetric part
       idxSym   = startSym : endSym;
       fprintf('pocs:Force setting the central line to be %d\n', centreLine, floor(sz(pfDim)/2)+1 );
    end
